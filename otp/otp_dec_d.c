...
int main(int argc, char *argv[]) {
	...
	if(argc < ARG_COUNT) {
		fprintf(stderr,"Usage: opt_dec_d <listening_port>\n");
		fflush(stderr);
		exit(1);
	}
	...
	if(sockfd < 0) {
		error("Decryption Server: ERROR opening socket");
	}
	...
	while(true) {
		...
		// The fork was successful -- continue on with reading and decoding
			...
			read(newsockfd, buffer, sizeof(buffer) - 1); // Read from the socket
			// Check to see if the read was successful
			if(strcmp(buffer, "dec_crh") != 0) {
			...
			// The read was successful -- continue
				...
				char answer[] = "dec_d_crh"; // Return the authorized response
				write(newsockfd, answer, sizeof(answer)); // Write to open socket
			}
			bzero(buffer, sizeof(buffer)); // Zero the buffer
			while(true) {
				...
				// If somehow we got negative bytes, print error and exit
					...
					error("Decryption Server: ERROR reading from socket");
				}
			...
			// Decode the message
			for(i = 0; message[i] != '\n'; i++) {
				n = charToInt(message[i]) - charToInt(key[i]); // Convert using the key
				if(n < 0) {
					n += 27; // Add 27 to get the character value
				}
